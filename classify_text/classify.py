...
class Classify(object):
    ...
    def __init__(self, corpus_path, seg_path, bag_path, test_bag_path):
        """
        :param corpus_path:
        :param seg_path:
        :param bag_path:
    ...
    def test_tf_idf(self, file_name):
        ...
        :return:
        """
        path = os.path.join(self.test_bag_path, 'test_set.dat')
        bunch = read_bunch(path)

        test_space = Bunch(target_name=bunch.target_name, label=bunch.label,
                           filenames=bunch.filenames, tdm=[], vocabulary={})
        train_bunch = read_bunch(os.path.join(self.bag_path, 'tfidfspace.dat'))
        ...
        space_path = os.path.join(self.test_bag_path, 'testspace.dat')
        save_bunch(space_path, test_space)

    def navie_alg(self):
        train_path = os.path.join(self.bag_path, 'tfidfspace.dat')
        train_set = read_bunch(train_path)

        test_path = os.path.join(self.test_bag_path, 'testspace.dat')
...
def run_classify(corpus_path, seg_path, bag_path, test_bag_path, test_corpus_path, test_seg_path):
    #classify = Classify(corpus_path, seg_path, bag_path, test_bag_path)
    #classify.save_seg_content()
    #classify.save_bunch('train_set')

    #classify.tf_idf(file_name='train_set')

    test_classify = Classify(test_corpus_path, test_seg_path, bag_path, test_bag_path)
